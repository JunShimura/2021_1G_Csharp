...
namespace Challenge03
{
    class Challenge03
    {
        ...
        static void Main(string[] args)
        {
            ...
            //　全パターン作成が完了
            // 全パターンのインデックスをシャッフル
            int[] randomId = new int[5040];
            Random random = new Random();
            for (var i = 0; i < randomId.Length; i++)
            {
                randomId[i] = i;
            }
            for (var i = 0; i < randomId.Length - 1; i++)
            {
                var r = random.Next(i, randomId.Length);    //入れ替える相手の指標
                //　交換
                var t = randomId[r];
                randomId[r] = randomId[i];
                randomId[i] = t;
            }
            for (var i = 0; i < 20; i++)
            {
                Console.Write($"\npattern#{i.ToString("00")}:");
                for (var j = 0; j < t.Length; j++)
                {
                    Console.Write($"{minoPattern[randomId[i], j]}");
                }
            }
        }
    }
